import {mainBackendDataEvent_20,
    mainBackendDataEvent_40,
    mainBackendDataEvent_60,
    mainBackendDataEvent_80,
    mainBackendDataEvent_100} from '../datas/mainDataEvent.js';
...
function calculateDiscountRate(cost:number, discountedCost:number) {
    if (cost <= 0) {
...
function convertToHundredUnits(amount:number) {
...
export class MainEvent {
    ...
    public eventDataBinding(index: number): void {
      const dataSource = dataMapping[index];
      
      if (dataSource) {
        const mainItemList = dataSource[0].data
        .map(({titleImage,hospitalSiGunGu,hospitalName,title,rating,ratingCount,discountedCost,cost})=>
            ({titleImage,hospitalSiGunGu,hospitalName,title,rating,ratingCount,discountedCost,cost}));
    ...
    private creating(itemList: MainItem[]): void {
      ...
      itemList.forEach(({titleImage,hospitalSiGunGu,hospitalName,title,rating,ratingCount,discountedCost,cost}) => {
        ...
        const insertHTML = `
            ...
            `;
        mainEl?.insertAdjacentHTML("beforeend", insertHTML);
